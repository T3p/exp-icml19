...
We use this tools to design a UCB-like algorithm for PO, called \algoname. The proposed algorithms apply to both policy optimization paradigms\footnote{We follow the taxonomy of~\citet{metelli2018policy}.}: action-based PO~\citep[the traditional one, see \eg][]{sutton2000policy} and parameter-based PO~\citep[where optimization is over \textit{distributions} of policies,][]{sehnke2008policy}, and both to finite and continuous parameter spaces. We show that these algorithms achieve $\wt{\mathcal{O}}(\sqrt{T})$ regret. Since the optimization side can be challenging in the continuous case, we propose a general purpose discretization method that allows to trade computational complexity with regret, preserving the sub-linearity of the latter anyway.
