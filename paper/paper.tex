...
Going further: for any $\zeta>0$, $\kappa=\frac{d}{\zeta}$ grants $O(t^{2+\zeta}d)$ time per iteration at the cost of $\wt{\mathcal{O}}\left(dT^{\left(1-\frac{\zeta}{d}\right)}\right)$ regret\footnote{The worse dependency $\mathcal{O}(d)$ of the regret on the arm space dimensionality (\wrt $\mathcal{O}(\sqrt{d})$ of Algorithm \ref{alg:1}) prevents the time per iteration from being $\mathcal{O}(d^d)$, irremediably exponential.}. 
