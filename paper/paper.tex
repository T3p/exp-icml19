...
In this section, we discuss some practical aspects of \algoname. From now on, we will focus on the case $\epsilon=1$, which is the only one of practical interest in the scope of this paper.

\subsection{Optimization}
Some of the most relevant applications of policy search (\eg robotics) naturally involve infinite arm sets $\Xspace$. As mentioned before, optimization (line 4 in Algorithm \ref{alg:1}) can be very challenging in this scenario, as the index $B_t(\vx,\delta_t)$ is non-convex and non-differentiable. Global optimization methods could be applied at the cost of giving up theoretical guarantees. This direction may be beneficial in practice, but we leave it to future, more application-oriented work. Instead, we propose a general discretization method, inspired by the proof of Theorem \ref{th:regretcompact}, which preserves the regret bound for the compact case. The key intuition, common in the continuous MAB literature, is to make the discretization progressively finer. The pseudocode for this variant, called \algoname2, is reported in Algorithm \ref{alg:2}. A reasonably coarse discretization can be used at the price of a worse (sublinear) regret:
%
\todomat{Theorem 4 da controllare!}
\begin{restatable}{theorem}{regretdiscretized}\label{th:regretdiscretized}
	Let $\Xspace$ be a $d$-dimensional compact arm set with $\Xspace \subseteq [-D,D]^d$. For any $\kappa\geq2$, under Assumptions \ref{ass:boundrenyi} and \ref{ass:lipschitz}, Algorithm \ref{alg:2} with confidence schedule $\delta_t = \frac{6\delta}{\pi^2t^2(1+t^{\nicefrac{d}{\kappa}})}$ and discretization schedule $\tau_t=\lceil t^{\frac{1}{\kappa}} \rceil$ guarantees, with probability at least $1-\delta$:
	\begin{align*}
	&\Reg(T) \leq \Delta_0  + C_2T^{\left(1-\frac{1}{\kappa}\right)}d
	\\&\quad+ C_2
	\sqrt{Tv_{1}
	\left(2(d+1)\log T + d\log d + \log \frac{\pi^2}{3\delta}\right)
	}
	\end{align*}
	where $C_1=2Lr$, $C_2=2\left(2\sqrt{2}+\frac{5}{3}\right)\norm[\infty]{f}$, and $\Delta_0$ is the instantaneous regret of the initial arm $\vx_0$.
\end{restatable}
%
For $\kappa=2$, we obtain regret $\wt{\mathcal{O}}(d\sqrt{T})$. Unfortunately, the time required for optimization is exponential in arm space dimensionality $d$. For $d\geq 2$, we can break the curse of dimensionality by taking $\kappa=d$. In this case, the regret is $\wt{\mathcal{O}}\left(dT^{\left(1-\frac{1}{d}\right)}\right)$. On the other hand, the time per iteration is only $O(t^3d)$. Note that the regret is sublinear for any choice of $\kappa$. 
Going further: for any $\epsilon>0$, $\kappa=\frac{d}{\epsilon}$ grants $O(t^{2+\epsilon}d)$ time per iteration at the cost of $\wt{\mathcal{O}}\left(dT^{\left(1-\frac{\epsilon}{d}\right)}\right)$ regret. 

\begin{algorithm}[t]
	\caption{\algoname2 ($\epsilon=1$)}
	\label{alg:2}
	\begin{algorithmic}[1]
...
\begin{proof}
	...
	\begin{align}\label{aux:1}
		&-c^{+}\beta_t{\vx}\leq \wc{\mu}(\vx) - \mu(\vx) \leq c^{-}\beta_t(\vx) & \text{for all $\vx\in\Xspace$ and $t=1,\dots,T$}.
	\end{align}
	Indeed:
	\begin{align}
		\Prob\left(\bigcap_{k=1}^{D}\bigcap_{t=1}^T\left[\wc{\mu}(\vx_k) - \mu(\vx_k) \leq c^{-}\beta_t(\vx_k)\right]\right) &=1-\Prob\left(\bigcup_{k=1}^{D}\bigcup_{t=1}^T\left[\wc{\mu}(\vx_k) - \mu(\vx_k) > c^{-}\beta_t(\vx_k)\right]\right)\nonumber\\
		&\geq 1-D\sum_{t=1}^{T}\Prob\left(\wc{\mu}(\vx_1) - \mu(\vx_1) > c^{-}\beta_t(\vx_1)\right) \label{pp:1}\\
		& \geq 1 - D\sum_{t=1}^{T}\delta_t\label{pp:2}\\
	...
	\begin{align*}
		\Prob\left(\bigcap_{k=1}^{D}\bigcap_{t=1}^T\left[\wc{\mu}(\vx_k) - \mu(\vx_k) \geq -c^{+}\beta_t(\vx_k)\right]\right) &=1-\Prob\left(\bigcup_{k=1}^{D}\bigcup_{t=1}^T\left[\wc{\mu}(\vx_k) - \mu(\vx_k) < -c^{+}\beta_t(\vx_k)\right]\right)\nonumber\\
		&\geq 1-D\sum_{t=1}^{T}\Prob\left(\wc{\mu}(\vx_1) - \mu(\vx_1) < -c^{+}\beta_t(\vx_1)\right)\\
		&\geq 1 - D\sum_{t=1}^{T}\delta_t\\
		 &\geq 1 - \frac{\delta}{2}.
	\end{align*}
	...
	\begin{align}
		...
		\wc{\mu}(\vx^*) + c^{+}\beta_t(\vx^*) - \mu(\vx_t) \label{pp:4}\\
		&\leq \wc{\mu}(\vx_t) + c^{+}\beta_t(\vx_t) - \mu(\vx_t) \label{pp:5}\\
		&\leq (c^{-} + c^{+})\beta(\vx_t) & \text{for all t=1,\dots,T}\label{aux:2}, 
	\end{align}
	where (\ref{pp:4}) and (\ref{aux:2}) are from (\ref{aux:1}), while (\ref{pp:5}) is by hypothesis, as $\vx_t=\arg\max_{\vx\in\Xspace}(\wc{\mu}(\vx) + c^{+}\beta_t(\vx))$.
	...
	\begin{align}
		...
		&=\Delta_0  + (c^{+}+c^{-})v_{\epsilon}^{\frac{\epsilon}{1+\epsilon}}\sum_{t=1}^{T}\left(\frac{2\log t + \log\frac{\pi^2D}{3\delta}}{t}\right)^{\frac{\epsilon}{1+\epsilon}} \label{pp:8}\\
		&\leq \Delta_0  + (c^{+}+c^{-})\left[v_{\epsilon}\left(2\log T + \log\frac{\pi^2D}{3\delta}\right)\right]^{\frac{\epsilon}{1+\epsilon}}\sum_{t=1}^{T}t^{-\frac{\epsilon}{1+\epsilon}} \nonumber\\
		&\leq \Delta_0  + (c^{+}+c^{-})\left[v_{\epsilon}\left(2\log T + \log\frac{\pi^2D}{3\delta}\right)\right]^{\frac{\epsilon}{1+\epsilon}}(1+\epsilon)T^{\frac{1}{1+\epsilon}} \label{pp:9},
	\end{align}
	...
	\begin{align}\label{aux:3}
		\sum_{t=1}^{T}t^{-\frac{\epsilon}{1+\epsilon}} \leq \int_{1}^{T+1}t^{-\frac{\epsilon}{1+\epsilon}}\de t
		= (1+\epsilon)\left((T+1)^{\frac{1}{1+\epsilon}} - 1\right) \leq (1+\epsilon)T^{\frac{1}{1+\epsilon}}.
	\end{align}
	The proof is concluded by renaming $C\gets (1+\epsilon)(c^{+}+c^{-}) = (1+\epsilon)(2\sqrt{2} + \frac{5}{3})\norm[\infty]{f}$.
\end{proof}
...
\begin{proof}
	...
	\begin{align}
		...
		& = \frac{G_1\Rmax}{(1-\gamma)^2} \coloneqq L, \label{pp:11}
	\end{align}
	where (\ref{pp:10}) is from the trivial fact $\norm[\infty]{Q_{\vtheta}}\leq\frac{\Rmax}{(1-\gamma)}$ and (\ref{pp:11}) is from assumption (\ref{eq:lp1}).
	...
	\end{align}
	where $\phi(s)$ is a vector of bounded state features, \ie $\sup_{s\in\Sspace}|\vphi(s)|\leq \vphi_{\max}$, satisfies assumption (\ref{eq:lp1}):
	\begin{align}
		...
		& \leq  \frac{2\vphi_{\max}}{\sqrt{2\pi}\sigma}\coloneqq G_1, \label{aux:5}
	\end{align}
	where (\ref{pp:14}) is by the substitution $x\gets \frac{a-\vtheta^T\vphi(s)}{\sigma}$. Even when $\sigma$ must be learned, proper parametrization (\eg $\sigma\propto\exp\{\vtheta\}$), together with the compactness of $\Theta$, allows to satisfy assumption (\ref{eq:lp1}).
	
	To show Lipschitz continuity for the parameter-based paradigm, it is enough to bound $\left|\nabla_{\vxi}\Exp_{\vtheta\sim\rho_{\vxi}}[J(\vtheta)]\right|$ under (\ref{eq:lp2}). For every $\vxi\in\Xi$:
	\begin{align}
		...
		&\leq \frac{G_2\Rmax}{(1-\gamma)} \coloneqq L \label{pp:13},
	\end{align}
	where (\ref{pp:12}) is from the trivial fact $J(\vtheta)\leq\frac{\Rmax}{1-\gamma}$, and (\ref{pp:13}) is from assumption (\ref{eq:lp2}). A Gaussian hyperpolicy $\rho_{\vxi}(\vtheta) = \mathcal{N}(\vxi,\boldsymbol{\sigma}I)$ satisfies assumption (\ref{eq:lp2}) with $G_2 = \frac{2}{\sqrt{2\pi}\boldsymbol{\sigma}}$. The proof of this fact is analogous to that of (\ref{aux:5}). Even when $\boldsymbol{\sigma}$ must be learned, proper parametrization (\eg $\boldsymbol{\sigma}\propto\exp\{\vtheta\}$), together with the compactness of $\Xi$, allows to satisfy assumption (\ref{eq:lp2}).
\end{proof}
