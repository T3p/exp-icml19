...
\begin{itemize}
	\item \enb{Parameter space} $\Theta \subseteq \Reals^d$\vfill
	\item<2-> A parametric \enb{policy} for each $\vtheta\in\Theta$\vfill
	\item<3-> Each inducing a distribution $p_{\vtheta}$ over \enb{trajectories}\vfill
	\item<4-> A \enb{return} $R(\tau)$ for every trajectory $\tau$\vfill
	\item<5-> {\large\enb{Goal:} $\max\limits_{\vtheta\in\Theta} J(\vtheta) = \Exp_{\tau\sim p_{\vtheta}}\left[R(\tau)\right]$}\vfill
	\item<6-> Iterative optimization (\eg gradient ascent)\vfill
\end{itemize}
\end{column}
...
\begin{itemize}
	\item<1-> \enb{Exploration-exploitation} trade-off\vfill
	\item<2-> The underlying Markov process is often \eno{continuous}\vfill
	\item<3-> \enb{Undirected} exploration: entropy bonus~\citep{haarnoja2018soft}\vfill
	\item<4-> \enb{Directed} exploration: pseudo-counts~\citep{bellemare2016unifying}\vfill
	\only<5->{
	\centering
	{\Large\eno{Lack} of theoretical guarantees}}\vfill
\end{itemize}
\end{frame}
