...
%		\item \textbf{Mixture} of behaviorals: $\vphi(z) = \frac{1}{K}\sum_{k=1}^K q_k(z)$
		...
		\begin{align*}
		&\wh{\mu}_{\text{BH}} \coloneqq \frac{1}{K}\sum_{k=1}^K
		\underbrace{
		\tikz[baseline]{\node[anchor=base,fill=orangep, inner sep=2pt](IW1){
			$\frac{p(z_{k})}{\Phi(z_k)}$
		};}}_{\mathclap{\textcolor{orange!70!black}{\bf Importance\,Weight (IW)}}}
		f(z_{k}),
...
%		\item Unbiased: $\mathbb{E}[\wh{\mu}_{\text{BH}}] = \mu$
		\item \enb{Unbiased}, but possibly \eno{high-variance}:
			\begin{align*}
				...
				&\quad d_{2}(p\|q) \coloneqq \int_{\mathcal{Z}}\left(\frac{p(z)}{q(z)}\right)^{2}\de z \qquad \textcolor{vert3}{\textbf{(R\'enyi divergence)}}
			\end{align*}
	\end{itemize}
}
...
\headerbox{Robust MIS Estimator}{name=robust,column=2,row=0,span=1, bottomaligned=mis}{
	...
	\compresslist
		...
		&\tikz[baseline]{\node[anchor=base,fill=bluep, inner sep=4pt](TR1){
			$\widecheck{\mu}_{\text{BH}}$};} 
			\coloneqq \frac{1}{K} \sum_{k=1}^K\min \left\{\underbrace{
			\tikz[baseline]{\node[anchor=base,fill=greenp, inner sep=2pt](TR1){
				$\sqrt{\frac{K d_{2}\left( p \| \Phi  \right)}{\log \frac{1}{\delta}}}
			$};}}_{\mathclap{\textcolor{green!70!black}{\bf truncation}}},  
			\underbrace{
			\tikz[baseline]{\node[anchor=base,fill=orangep, inner sep=2pt](IW2){
				$\vphantom{\sqrt{\frac{K d_{2}\left( p \| \Phi  \right)}{\log \frac{1}{\delta}}}}\frac{p(z_{k})}{\Phi(z_k)}
			$};}}_{\textcolor{orange!70!black}{\bf IW}} \right\} f(z_{k})
		\end{align*}
		...
		\tikz[baseline]{\node[anchor=base,fill=greenp, inner sep=1pt](BON1){
			...
			\|f\|_{\infty} \left(\sqrt{2} + \frac{4}{3} \right)  \sqrt{\frac{ d_{2}\left( p \| \Phi  \right) \log  \frac{1}{\delta}  }{K}}$};}
		\end{align*}
	\end{itemize}
}
...
\compresslist
	...
	\begin{align*}
		...
		&\underbrace{
			\tikz[baseline]{\node[anchor=base,fill=bluep, inner sep=3pt](EST1){
		...
		+ 
		&\underbrace{
			\tikz[baseline]{\node[anchor=base,fill=greenp, inner sep=3pt](BON2){
			$\norm[\infty]{f}\left(\sqrt{2}+\frac{4}{3}\right)
			\sqrt{\frac{d_{1+\epsilon}(p_{\vx}\|\Phi_{t})\log\frac{1}{\delta_t}}{t}}
