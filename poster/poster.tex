...
\headerbox{Policy Optimization}{name=polopt,column=0,row=1,span=1, below=problem}{
	...
	\compresslist
		...
		\item \textbf{Return} $\Rew(\tau) = \sum_{h=0}^{H-1}\gamma^h r_{h+1}$
	\end{itemize}
	\centering
	
	\tikzstyle{background rectangle}=[thick, draw=blue!50]
	\noindent \begin{tikzpicture}[show background rectangle]
		\node[align=justify, text width=0.85\textwidth, inner sep=0.8em]{
		\vspace{-0.65cm}
	\begin{center}
	...
	\begin{itemize}[leftmargin=*]
		\item \small Induced \textbf{trajectory distribution} \vspace{-0.3cm} 
	\end{itemize}
	$$ \tau \sim p_{\vtheta}(\tau)$$
	\end{minipage}
	\vspace{0.4cm}\\
	\textcolor{blue}{\textbf{Goal}}: $\displaystyle \vtheta^*=\arg\max_{\vtheta\in\Theta} J(\vtheta)\coloneqq \mathbb{E}_{\tau\sim p_{\vtheta}}[\Rew(\tau)]$ \vspace{-0.3cm}
	\end{center}
	};
	...
	\end{minipage}
	\vspace{0.4cm}\\
	\textcolor{red}{\textbf{Goal}}: $\displaystyle \vxi^* = \arg\max_{\vxi\in\Xi}J(\vxi) \coloneqq \mathbb{E}_{\vtheta\sim\nu_{\vxi}}\left[J(\vtheta)\right]$\vspace{-0.3cm}
	\end{center}};
...
\headerbox{Policy Optimization as Correlated MAB}{name=ucb,column=0,row=2,span=1, above=bottom}{
	...
	\tikzstyle{background rectangle}=[thick, draw=black!50]
	\noindent \begin{tikzpicture}[show background rectangle]
		\node[align=justify, text width=0.82\textwidth, inner sep=0.8em]{
		\vspace{-0.3cm}
		\begin{itemize}[leftmargin=*]
		\compresslist
		...
		\end{itemize}
		%_{\vx_0,\vx_1,...,\vx_T \in \Xspace}
		\textcolor{black}{\textbf{Goal}}: $\min_{\vx_0,...,\vx_T \in \Xspace} \mathop{Regret}(T)$
	};
	\node[overlay, fill=white, draw=white, above] at (current bounding box.north) {\textcolor{black}{\textbf{MAB jargon}}};
...
%		\item \textbf{Mixture} of behaviorals: $\vphi(z) = \frac{1}{K}\sum_{k=1}^K q_k(z)$
		...
		\underbrace{
		\tikz[baseline]{\node[anchor=base,fill=orangep!50, inner sep=2pt](IW1){
			$\frac{p(z_{k})}{\Phi_K(z_k)}$
		};}}_{\mathclap{\textcolor{orangep!90!black}{\bf Importance\,Weight (IW)}}}
		f(z_{k}),
		&\;\;\underbrace{\Phi_K(z) = \frac{1}{K}\sum_{k=0}^{K-1} q_k(z)}_{\text{mixture}}
		\end{align*}
%		\item Unbiased: $\mathbb{E}[\wh{\mu}_{\text{BH}}] = \mu$
		\item \enb{Unbiased}, but possibly \eno{high-variance}:
			\begin{align*}
				...
				&\quad d_{2}(p\|q) \coloneqq \int_{\mathcal{Z}}q(z)\left(\frac{p(z)}{q(z)}\right)^{2}\de z \quad \textcolor{vert3}{\textbf{(R\'enyi divergence)}}
			\end{align*}
	\end{itemize}
}
...
\headerbox{Robust MIS Estimator}{name=robust,column=2,row=0,span=1, bottomaligned=mis}{
	...
	\compresslist
		...
		&\tikz[baseline]{\node[anchor=base,fill=bluep, inner sep=4pt](EST1){
			...
			\underbrace{
			\tikz[baseline]{\node[anchor=base,fill=orangep!50, inner sep=2pt](IW2){
...
%				\sqrt{\frac{d_{1+\epsilon}(p_{\vx}\|\Phi_{t})\log\frac{1}{\delta_t}}{t}}}
	...
	\item \normalsize Select arm $\vx_{t} = \arg\max_{\vx\in\Xspace} B_t(\vx,\delta_t)$
	\item \normalsize Draw outcome $z_t\sim p_{\vx_t}$ and observe payoff $f(z_t)$
\end{itemize}
\end{itemize}
\tikz[overlay]{
...
\headerbox{Regret Analysis}{name=regret,column=1,row=2,span=2, below=ucb}{
	...
	\compresslist
		...
		\begin{itemize}
		\compresslist
			\item \underline{Assumptions}: \emph{uniformly} bounded R\'enyi divergence $d_{2}(p_{\vx}\|\Phi) \leq v$
		...
		\begin{itemize}
		\compresslist
			\item \underline{Assumptions}: \emph{uniformly} bounded R\'enyi divergence $d_{2}(p_{\vx}\|\Phi) \leq v$, $L$-Lipschitz objective $\mu$
...
%	\end{tabular}
%	
	\begin{tabular}{ L{3.5cm} C{2cm} C{2cm} C{3.5cm} C{2cm} C{3.5cm}} 
		...
		\textcolor{blue}{\textbf{Action-based PO}} & $\vtheta \in \Theta$ & $\tau \in \mathcal{T}$ & $p_{\vtheta}(\tau)$ & $\Rew(\tau)$ & $J(\vtheta)$ \\
		\textcolor{red}{\textbf{Parameter-based PO}} & $\vxi \in \Xi$ & $\vtheta \in \Theta$ & $\nu_{\vxi}(\vtheta)$ & $J(\vtheta)$ & $J(\vxi)$ \\
		\hline
	\end{tabular}
