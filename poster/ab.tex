...
\begin{tikzpicture}[node distance = 0.8cm, auto, >=latex,
   ...
   every text node part/.style={align=center}]
    ...
    6em, color=black, align=left] (pi) {$\qquad\qquad \pi_{\bm{\theta}}$ \\ $ $ \\ $ $ \\ $ $  };
    
    \node [right = of pi] (a) {$a$};
    
	\def\normal{\x,{1.5*exp(-8*(\x)^2)}};
	\draw [color=blue, domain=-1:1, samples=100, line width=1pt,yshift=-22] plot (\normal);
   	
   	\draw[->,densely  dashed] (pi) to [looseness=1.2] node[above] {} (a);
   	\draw[->] ($(pi) + (-1.5,0)$)  node[left] {$s$} to (pi);
   	%\draw[->] (pi) to ($(pi) + (+1.5,0)$) node[right] {$a$} ;
